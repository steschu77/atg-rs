...
// ----------------------------------------------------------------------------
fn show_bits(bp: &usize, src: &[u8], count: usize) -> Result<u16> {
    let bytepos = *bp >> 3;
    let shift = *bp & 7;
    let mask = (1 << count) - 1;
...
// ----------------------------------------------------------------------------
fn read_bits(src: &[u8], sptr: &mut usize, count: usize) -> Result<u16> {
...
fn generate_codes(codes: &mut [u16], lengths: &[u8]) -> Result<bool> {
...
// ------------------------------------------------------------------------
fn make_lookup_table(lengths: &[u8]) -> Result<LookupTable> {
...
// ----------------------------------------------------------------------------
fn read_symbol(src: &[u8], sptr: &mut usize, lookup_table: &LookupTable) -> Result<u16> {
    let idx = show_bits(sptr, src, 9)? as usize;
    ...
    if code0.len <= TABLE_BITS {
        ...
        *sptr += TABLE_BITS as usize;
        let count = (code0.len - TABLE_BITS) as usize;

        let idx = show_bits(sptr, src, count)? as usize;
        let code = &lookup_table[code0.code as usize + idx];
...
// ----------------------------------------------------------------------------
fn generate_fixed_luts() -> Result<(LookupTable, LookupTable)> {
...
// ----------------------------------------------------------------------------
fn read_encoded_luts(src: &[u8], sptr: &mut usize) -> Result<(LookupTable, LookupTable)> {
...
// ----------------------------------------------------------------------------
pub fn inflate(dst: &mut [u8], src: &[u8]) -> Result<usize> {
    let mut sptr = 0;
    let mut dptr = 0;
    loop {
